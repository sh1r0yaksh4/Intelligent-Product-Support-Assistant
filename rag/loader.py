...
def _read_pdf(path: Path) -> Iterable[tuple[str, str, int]]:
    reader = PdfReader(str(path))
    for page_number, page in enumerate(reader.pages, start=1):
        text = clean_text(page.extract_text() or "")
        if text:
            yield f"Page {page_number}", text, page_number


def load_file(path: Path, product_id: str, source_url: str = "") -> list[DocumentChunk]:
    """Return chunks with stable source metadata from a PDF, TXT, or Markdown file."""
    ...
    if extension == ".pdf":
        sections = ((title, text, page) for title, text, page in _read_pdf(path))
    elif extension in {".txt", ".md"}:
        text = path.read_text(encoding="utf-8", errors="ignore")
        faq_pairs = _faq_blocks(text)
        if faq_pairs:
            sections = ((question, f"Question: {question}\nAnswer: {answer}", None) for question, answer in faq_pairs)
            base_metadata["document_type"] = "faq"
        else:
            sections = ((title, body, None) for title, body in _markdown_sections(text))
    else:
        return []
    ...
    for section, text, page in sections:
        for index, part in enumerate(_chunk_words(text)):
            ...
            metadata.update({"section": section, "page": page or "", "chunk_index": index})
